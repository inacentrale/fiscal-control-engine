...
class CorporateIncomeTaxXmlExtractor:
    def extract(
        self,
        source_path: Path,
        ...
        try:
            ...
            raise CorporateIncomeTaxTabularExtractionError(
                "IS XML source cannot be read",
            ) from exc
        if any(marker in content.upper() for marker in _FORBIDDEN_XML_MARKERS):
            raise CorporateIncomeTaxTabularExtractionError("unsafe IS XML source")
        try:
            root = ElementTree.fromstring(content)
        except ElementTree.ParseError as exc:
            raise CorporateIncomeTaxTabularExtractionError(
                "invalid IS XML source",
            ) from exc
        rows = _candidate_rows(root)
        if not rows:
            raise CorporateIncomeTaxTabularExtractionError(
                "IS XML structure is unresolved",
            )
        return CorporateIncomeTaxTabularExtractor().extract_frame(
            pd.DataFrame(rows),
            source_reference,
        )
