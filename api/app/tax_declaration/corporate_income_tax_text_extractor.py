...
class CorporateIncomeTaxDocumentExtractor:
    def __init__(
        self,
    ...
    def extract_text(
        ...
        *,
        ...
        declaration = CorporateIncomeTaxTabularExtractor().extract_frame(
...
def _explicit_table(text: str) -> pd.DataFrame:
    ...
    for index, header in enumerate(rows):
        ...
        if (
            ...
            or "benefice" not in normalized_header
        ):
            continue
        body = [row for row in rows[index + 1 :] if len(row) == len(header)]
        if body:
            return pd.DataFrame(body, columns=header)
    raise CorporateIncomeTaxTabularExtractionError("IS document table is unresolved")
