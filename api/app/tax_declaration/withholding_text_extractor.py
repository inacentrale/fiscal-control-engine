...
class WithholdingDocumentExtractor:
    def __init__(
        self,
    ...
    def extract_text(
        ...
        *,
        ...
        dataframe = _explicit_table(text)
        declaration = WithholdingTabularExtractor().extract_frame(
            dataframe,
            source_reference,
        )
...
def _explicit_table(text: str) -> pd.DataFrame:
    ...
    for index, header in enumerate(rows):
        ...
        if (
            ...
            or "retenue" not in normalized_header
        ):
            continue
    ...
    raise WithholdingTabularExtractionError(
        "withholding document table is unresolved",
    )
