...
class VatXmlExtractor:
    def extract(
        ...
        try:
            ...
            raise VatTabularExtractionError("VAT XML source cannot be read") from exc
        upper_content = content.upper()
        if any(marker in upper_content for marker in _FORBIDDEN_XML_MARKERS):
            raise VatTabularExtractionError("unsafe VAT XML source")
        try:
            root = ElementTree.fromstring(content)
        except ElementTree.ParseError as exc:
            raise VatTabularExtractionError("invalid VAT XML source") from exc

        rows = _candidate_rows(root)
        if not rows:
            raise VatTabularExtractionError("VAT XML structure is unresolved")
        dataframe = pd.DataFrame(rows)
        return VatTabularExtractor().extract_frame(dataframe, source_reference)
